...
> ## ⚠️ 実装凍結中
>
> **このドキュメントは設計文書です。実装は凍結されています。**
>
> - **理由**: 物理的モジュール分割が最優先。内部構造が安定するまで API 実装は行わない。
> - **実装予定**: Post v2.0.0（内部リファクタリング完了後）
> - **このドキュメントの位置づけ**: 将来の到達点として設計を保持
...
| フェーズ | pme API の扱い |
|----------|---------------|
| alpha.1 (Phase 2-A) ✅ | 観測のみ |
| alpha.2/3 (Phase 2-B/C) | **設計完了、実装凍結** |
| beta (Phase 3) | 物理的モジュール分割を優先 |
| RC | 物理的モジュール分割を完了 |
| v2.0.0 | リリース（API 実装なし） |
| Post v2.0.0 | API 実装開始、全て **Experimental** |
| v2.1.0+ | 利用実績を見て **Stable** に昇格 |

### なぜ実装を凍結するか

1. **物理的モジュール分割が最優先**: 内部構造を整理しないと API 設計が空回りする
2. **内部構造が安定していない**: Reload Scripts 問題、props ライフサイクル問題が未解決
3. **リファクタの自由度**: API を実装すると、内部変更の選択肢が狭まる
4. **利用実績がない**: 外部ツールからのフィードバックを得てから API を固めたい
