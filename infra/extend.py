...
class ExtendManager:
    ...
    def swap_order(self, pm_uid: str, direction: int) -> bool:
        ...
        Args:
            pm_uid: The PM's unique identifier.
            direction: -1 for inner (toward original), +1 for outer.

        Returns:
            True if swapped, False if at boundary or not found.
        """
        entry = self._entries.get(pm_uid)
        if not entry:
            return False

        entries = self.get_entries(entry.target, entry.side)
        current_idx = next(
            (i for i, e in enumerate(entries) if e.pm_uid == pm_uid), -1
        )
        if current_idx == -1:
            return False
        ...
        # Boundary check
        if neighbor_idx < 0 or neighbor_idx >= len(entries):
            return False

        # Swap orders
        neighbor = entries[neighbor_idx]
        entry.order, neighbor.order = neighbor.order, entry.order

        # Refresh display (combined func's internal sort will update)
        self._refresh_combined(entry.target, entry.side)

        return True

    def cross_boundary(self, pm_uid: str) -> bool:
        """Move a PM from prepend to append or vice versa (innermost position).

        Args:
            pm_uid: The PM's unique identifier.

        Returns:
            True if crossed, False if not found.
        """
        entry = self._entries.get(pm_uid)
        if not entry:
            return False
        ...
        old_side = entry.side
        new_side = "append" if old_side == "prepend" else "prepend"

        # Update entry
        entry.side = new_side
        entry.order = 0  # Innermost on the new side

        # Refresh both sides
        self._refresh_combined(entry.target, old_side)
        self._refresh_combined(entry.target, new_side)

        # Normalize orders on the new side
        self._normalize_orders(entry.target, new_side)
