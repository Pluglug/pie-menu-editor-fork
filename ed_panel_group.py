...
class PME_OT_toolbar_menu(bpy.types.Operator):
    ...
    def draw_toolbar_menu(self, menu, context):
        ...
        lh.operator(
            self.bl_idname, "Create Toolbar (Current Screen)", 'ADD', name=dir_scr_name
        )
        lh.operator(
            self.bl_idname, "Create Toolbar (All Screens)", 'ADD', name=dir_name
        )
...
class PME_PT_toolbar(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if is_28():
            margin = round(4 * get_uprefs().view.ui_scale)
            c_layout.y += margin
        else:
            margin = round(3 * get_uprefs().view.ui_scale)
        ...
        has_menu = (
            draw_menu(dir_scr_name)
            or draw_menu(dir_name)
            or draw_menu(scr_name)
            or draw_menu(def_name)
        )
        ...
        if not has_menu:
            ...
            lh.operator(
                ...
                "" if vertical else "Pie Menu Editor",
                'COLOR',
            )
