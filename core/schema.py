# core/schema.py - PME Menu Schema Registry
# LAYER = "core"
#
# This module provides the schema registry system for PME.
# It handles registration, parsing, and encoding of PM/PMI properties.
#
# Renamed from: core/props.py (Phase 8-C: Schema Rename)
#
# Contains:
#   - SchemaProp: Single property metadata (formerly PMEProp)
#   - SchemaRegistry: Property registry and parser (formerly PMEProps)
#   - ParsedData: Parsed property container
#   - schema: Global SchemaRegistry instance
#
# Design note:
...
class SchemaProp:
    """Single property metadata (formerly PMEProp).
...
class SchemaRegistry:
    """PME Menu Schema Registry (formerly PMEProps).
    ...
    def IntProperty(self, type, name, default=0):
        ...
        self.prop_map[name] = SchemaProp(type, name, default, 'INT')

    def BoolProperty(self, type, name, default=False):
        """Register a boolean property."""
        self.prop_map[name] = SchemaProp(type, name, default, 'BOOL')

    def StringProperty(self, type, name, default=""):
        """Register a string property."""
        self.prop_map[name] = SchemaProp(type, name, default, 'STR')

    def EnumProperty(self, type, name, default, items):
        """Register an enum property."""
        self.prop_map[name] = SchemaProp(type, name, default, 'STR', items)
...
# Global schema instance - must be defined before ParsedData
...
class ParsedData:
    ...
    def __init__(self, text):
        ...
        for k, prop in schema.prop_map.items():
        ...
        for prop in data:
            if not prop:
                continue
            k, v = prop.split("=")
            if k in schema.prop_map:
                setattr(self, k, schema.prop_map[k].decode_value(v))
        ...
        self._initialized = bool(schema.prop_map)
    ...
    # See: grep -r "schema\.(Enum|String|Int|Bool)Property" for all registrations
    _FALLBACK_DEFAULTS = {
